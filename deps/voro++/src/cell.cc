...
void voronoicell_base::draw_povs(double x,double y,double z,FILE* fp) {
	int i,j,k;double *ptsp=pts,*pt2;
	double x1,y1,z1,x2,y2,z2,r,capacity=1.0e-5;
	char posbuf1[128],posbuf2[128];
	for(i=0;i<p;i++,ptsp+=3) {
		x1=x+*ptsp*0.5; y1=y+ptsp[1]*0.5; z1=z+ptsp[2]*0.5;
		sprintf(posbuf1,"%g,%g,%g",x1,y1,z1);
		fprintf(fp,"\tsphere{ <%s>,r_bond texture{t_bond} }\n",posbuf1);
		for(j=0;j<nu[i];j++) {
			k=ed[i][j];
			if(k<i) {
				pt2=pts+3*k;
				x2=x+*pt2*0.5;y2=y+pt2[1]*0.5;z2=z+pt2[2]*0.5;
				r=pow(x2-x1,2.0)+pow(y2-y1,2.0)+pow(z2-z1,2.0);
				sprintf(posbuf2,"%g,%g,%g",x2,y2,z2);
				if(r>capacity) fprintf(fp,"\tcylinder{ <%s>,<%s>,r_bond texture{t_bond} }\n",posbuf1,posbuf2);
			}
		}
	}
}
...
void voronoicell_base::normals_with_edges(std::vector<double> &v, std::vector<double> &w) {
	int i,j,k;
	v.clear(); w.clear();
	for(i=1;i<p;i++) for(j=0;j<nu[i];j++) {
		k=ed[i][j];
		if(k>=0) normals_with_edges_search(v,w,i,j,k);
	}
	reset_edges();
}

inline void voronoicell_base::normals_with_edges_search(std::vector<double> &v,std::vector<double> &w,int i,int j,int k) {
	ed[i][j]=-1-k;
	int l=cycle_up(ed[i][nu[i]+j],k),m,ip;
	double ux,uy,uz,vx,vy,vz,wx,wy,wz,wmag,w1[3],w2[3],w3[3];
	for(ip=0;ip<3;ip++){
		w1[ip]=0;w2[ip]=0;w3[ip]=0;
	}
	do {
		m=ed[k][l];ed[k][l]=-1-m;
		ux=pts[3*m]-pts[3*k];
		...
		if(ux*ux+uy*uy+uz*uz>tolerance_sq) {
			...
			while(m!=i) {
				...
				w2[0]=vx;w2[1]=vy;w2[2]=vz;
				w3[0]=pts[3*m];w3[1]=pts[3*m+1];w3[2]=pts[3*m+2];
				...
				if(wmag>tolerance_sq) {
					...
					for(ip=0;ip<3;ip++){ w.push_back(w1[ip]); }
					for(ip=0;ip<3;ip++){ w.push_back(w2[ip]); }
					for(ip=0;ip<3;ip++){ w.push_back(w3[ip]); }
