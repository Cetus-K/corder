...
class container_periodic : public container_periodic_base, public radius_mono {
	public:
		...
		int mytype_povs(int myid,int ntypes,std::vector<int> item){
			int a,ret,id_start,id_end;
			id_start = 0;
			for( a=0; a<ntypes; a++ ){
				id_end=id_start+item[a]-1;
				if( id_start <= myid && myid <= id_end ){ ret = a; break; }
				id_start += item[a];
			}
			return ret; 
		}
		template<class c_loop>
		void draw_particles_povs(c_loop &vl,FILE *fp,int ntypes,std::vector<int> item) {
			int a; double *pp;
			if(vl.start()) do {
				pp=p[vl.ijk]+3*vl.q; a=mytype_povs(id[vl.ijk][vl.q],ntypes,item)+1;
				fprintf(fp,"\tsphere{ <%g,%g,%g>,s%d texture{t%d} }\n",*pp,pp[1],pp[2],a,a);
			} while(vl.inc());
		}
		inline void draw_particles_povs(int ntypes,std::vector<int> item,FILE *fp=stdout) {
			c_loop_all_periodic vl(*this);
			draw_particles_povs(vl,fp,ntypes,item);
		}
		inline void draw_particles_povs(const char *filename,int iteration,int start,int ntypes,std::vector<int> item,int samples) {
			int j; FILE *fp;
			if( iteration == start ){
				fp = safe_fopen(filename,"w");
				for( j=0; j<ntypes; j++ ) fprintf(fp,"#local s%d = 0.6;\n",j+1);
				for( j=0; j<ntypes; j++ ) fprintf(fp,"#local t%d = texture{ pigment{ rgb<0.3,1,1> } };\n",j+1);
				fprintf(fp,"#declare config = array[%d]\n",samples);
			}
			else{ fp = safe_fopen(filename,"a"); }
			fprintf(fp,"\n");
			fprintf(fp,"#declare config[%d] =\n",iteration-start);
			fprintf(fp,"union{\n");
			draw_particles_povs(ntypes,item,fp);
			fprintf(fp,"}\n");
			fclose(fp);
		}
		template<class c_loop>
		void draw_cells_povs(c_loop &vl,FILE *fp) {
			voronoicell c;double *pp;
			if(vl.start()) do if(compute_cell(c,vl)) {
				fprintf(fp,"\t// cell %d\n",id[vl.ijk][vl.q]+1);
				pp=p[vl.ijk]+ps*vl.q;
				c.draw_povs(*pp,pp[1],pp[2],fp);
			} while(vl.inc());
		}
		inline void draw_cells_povs(FILE *fp=stdout) {
			c_loop_all_periodic vl(*this);
			draw_cells_povs(vl,fp);
		}
		inline void draw_cells_povs(const char *filename,int iteration,int start,int samples) {
			FILE *fp;
			if( iteration == start ){
				fp = safe_fopen(filename,"w");
				fprintf(fp,"#local r_bond = 0.07;\n");
				fprintf(fp,"#local t_bond = texture{ pigment{ rgb<1,1,1> } };\n");
				fprintf(fp,"#declare cell_config = array[%d]\n",samples);
			}
			else{ fp = safe_fopen(filename,"a"); }
			fprintf(fp,"\n");
			fprintf(fp,"#declare cell_config[%d] =\n",iteration-start);
			fprintf(fp,"union{\n");
			draw_cells_povs(fp);
			fprintf(fp,"}\n");
			fclose(fp);
